...
def run_scenario_a(params: Params, seed: int = 42) -> pd.DataFrame:
    ...
    [FIXED: xm_range=(1.0, 1.0) ensures all messages have Xm = +1]
    """
    n = 200
...
def run_scenario_b(params: Params, seed: int = 42) -> pd.DataFrame:
    ...
    [FIXED: friction_reduce now applies FRi <- clip(FRi - 0.2, 0, 1) to each agent's state.fr
            (agent-level friction, Table 1 / Eq.26), NOT a change to message fr_range]
    """
    n = 200
    n_steps = 60
    g = nx.watts_strogatz_graph(n=n, k=6, p=0.05, seed=seed)
    ...
    # Condition 3: Friction reduction — modify agent-level FRi [FIXED]
...
def run_scenario_c(params: Params, seed: int = 42) -> pd.DataFrame:
    ...
    [FIXED: identity_congruence() now uses Pi and Xm; segregation threshold corrected]
    """
    n = 200
    n_steps = 60
    g = nx.watts_strogatz_graph(n=n, k=6, p=0.05, seed=seed)
    ...
    for cond, gamma1_val in [("no_amplification", 0.0), ("amplification", 2.0)]:
        ...
        traits_c, states_c, grps = init_population_identity_groups(n, np.random.default_rng(seed))
        ...
        all_dfs.append(outputs_to_frame_with_groups(out).assign(condition=cond))
...
def main():
    ...
    if args.scenario_a:
        ...
        for route in ["central", "peripheral"]:
            ...
            peak   = sub.max()
            peak_t = int(np.argmax(sub))
            end    = sub[-1]
            ret    = end / peak if peak > 0 else float("nan")
            hl     = durability_half_life(sub)
