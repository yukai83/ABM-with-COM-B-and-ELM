...
def run_scenario_a(params: Params, seed: int = 42) -> pd.DataFrame:
    """Central vs peripheral route durability.

    Two 50-step runs with campaign messages up to step 20, then a passive
    observation phase. Central-route condition uses high-AQ messages and
    low initial load; peripheral-route uses weaker arguments, stronger cues,
    and high initial load to push agents toward peripheral processing.
    """
    n = 200
...
def run_scenario_b(params: Params, seed: int = 42) -> pd.DataFrame:
    """Feasibility-constrained behaviour across capability quartiles.

    Same population, three conditions: baseline, cap boost for Q1 (+0.2),
    and reduced friction (fr_range capped at 0.3). Per-quartile behaviour
    rates reveal that attitude change is insufficient when COM-B gates are closed.
    """
    n = 200
    n_steps = 60
    g = nx.watts_strogatz_graph(n=n, k=6, p=0.05, seed=seed)
    ...
    q25, q50, q75 = np.percentile(cap_vals, [25, 50, 75])
    groups = {}
    for i in range(n):
        c = states_base[i].cap
        if   c <= q25: groups[i] = "Q1"
        elif c <= q50: groups[i] = "Q2"
        elif c <= q75: groups[i] = "Q3"
        else:          groups[i] = "Q4"
    ...
    all_dfs.append(outputs_to_frame_with_groups(out_1).assign(condition="baseline"))
    ...
    all_dfs.append(outputs_to_frame_with_groups(out_2).assign(condition="cap_boost_Q1"))
    ...
    all_dfs.append(outputs_to_frame_with_groups(out_3).assign(condition="friction_reduce"))
...
def run_scenario_c(params: Params, seed: int = 42) -> pd.DataFrame:
    """Amplification and emergent clustering.

    Two identity groups (G0: negative identity direction preference,
    G1: positive) exposed to messages spanning the full identity_dir range.
    Comparing gamma1=0 vs gamma1=2.0 shows how engagement-driven amplification
    drives attitude divergence and biased exposure patterns.
    """
    n = 200
    n_steps = 60
    g = nx.watts_strogatz_graph(n=n, k=6, p=0.05, seed=seed)
    ...
    for condition, gamma1_val in [("no_amplification", 0.0), ("amplification", 2.0)]:
        ...
        traits, states, groups = init_population_identity_groups(n, np.random.default_rng(seed))
        out, _ = simulate(
        ...
        all_dfs.append(outputs_to_frame_with_groups(out).assign(condition=condition))
...
def main():
    ...
    print(
        f"Final beh_rate: {df['beh_rate'].iloc[-1]:.3f} | "
