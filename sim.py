...
def delta_attitude(state, traits, msg, pcent, p):
    ...
    # Directional resistance: agents near ceiling/floor are harder to push further
...
class StepOutputs:
    ...
    # Extended outputs for Scenarios B and C
    att_variance: float = 0.0                        # Scenario C: polarisation measure
    group_beh_rates: Dict[str, float] = field(default_factory=dict)   # Scenario B: per-quartile rates
    group_mean_atts: Dict[str, float] = field(default_factory=dict)   # Scenario C: per-group attitudes
    exposure_segregation: float = float("nan")       # Scenario C: filter-bubble proxy
...
def simulate(
    ...
    groups: optional dict mapping agent_id -> group label string.
        When provided, StepOutputs.group_beh_rates and group_mean_atts
        are populated each step. Used by Scenario B (capability quartiles)
        and Scenario C (identity groups).

    track_segregation: when True, computes exposure_segregation each step
        as the fraction of exposures that are identity-congruent (ic > 0).
        Used by Scenario C to measure filter-bubble effects.
    """
    outputs = []
    shares_prev = {}
    next_msg_id = 0

    # Pre-compute unique group labels once
    ...
    for t in range(n_steps):
        ...
        # Segregation counters (Scenario C)
        seg_aligned = 0
        seg_total = 0
        ...
        for i in states.keys():
            ...
            for m in msgs:
                ...
                if rng.uniform() < p_exp:
                    ...
                    # Scenario C: count identity-aligned vs total exposures
        ...
        # Aggregate population-level arrays
        ...
        # Per-group metrics (Scenarios B and C)
        ...
        # Exposure segregation (Scenario C)
...
def init_population_identity_groups(n_agents: int, rng: np.random.Generator):
    """
    Scenario C variant: population split into two identity groups.
    Group 'G0': low trust_inst (0.1-0.35) → agent_dir ≈ negative → prefers negative identity_dir messages.
    Group 'G1': high trust_inst (0.65-0.90) → agent_dir ≈ positive → prefers positive identity_dir messages.
    High identity_salience for both groups so the grouping has real bite.
    Returns traits, states, groups dict.
    """
    from .models import AgentTraits, AgentState
    traits = {}
    states = {}
    ...
    for i in range(n_agents):
        ...
        # Identity direction strongly determined by group
        if grp == "G0":
        ...
        tr = AgentTraits(
            ...
            trust_peer=float(rng.uniform(0.2, 0.8)),
            identity_salience=float(rng.uniform(0.55, 0.85)),  # high salience
        )
        st = AgentState(
            cap=float(rng.uniform(0.3, 0.8)),
            opp=float(rng.uniform(0.3, 0.8)),
