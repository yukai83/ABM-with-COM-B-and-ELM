...
def verify_route_monotonicity(params: Params, seed: int = 42) -> str:
    """High-NFC agents should have higher p_central than low-NFC agents (Eq.12)."""
    n = 50
    ...
    low_nfc = {
        i: AgentTraits(nfc=0.1, trust_inst=traits[i].trust_inst,
    ...
    high_nfc = {
        i: AgentTraits(nfc=0.9, trust_inst=traits[i].trust_inst,
                       trust_peer=traits[i].trust_peer,
                       identity_salience=traits[i].identity_salience,
                       pi=traits[i].pi)
        for i in agent_ids[n // 2:]
    ...
    low_pc  = float(np.mean([p_central(states[i], low_nfc[i],  msg, params) for i in agent_ids[:n // 2]]))
    high_pc = float(np.mean([p_central(states[i], high_nfc[i], msg, params) for i in agent_ids[n // 2:]]))
...
def verify_ic_formula(params: Params, seed: int = 42) -> str:
    """ICi,m = 1 - |Pi - Xm| / 2: fully congruent should give 1.0, incongruent 0.0."""
    ...
    ic_pp = identity_congruence(tr_pos, msg_pos)   # expect 1.0
    ic_pn = identity_congruence(tr_pos, msg_neg)   # expect 0.0
    ic_np = identity_congruence(tr_neg, msg_pos)   # expect 0.0
    ic_nn = identity_congruence(tr_neg, msg_neg)   # expect 1.0

    ok = (abs(ic_pp - 1.0) < 1e-9 and abs(ic_pn) < 1e-9
          and abs(ic_np) < 1e-9 and abs(ic_nn - 1.0) < 1e-9)

    return (f"ic_formula: {'PASS' if ok else 'FAIL'} | "
            f"ic(+,+)={ic_pp:.3f}  ic(+,-)={ic_pn:.3f}  "
            f"ic(-,+)={ic_np:.3f}  ic(-,-)={ic_nn:.3f}")


def ablation_visibility(params: Params, seed: int = 42) -> str:
    """gamma1=2 should produce higher mean attitude than gamma1=0."""
    n = 100
    g = nx.watts_strogatz_graph(n=n, k=4, p=0.05, seed=seed)
